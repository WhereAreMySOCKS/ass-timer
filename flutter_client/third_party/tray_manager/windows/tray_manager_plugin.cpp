...
class TrayManagerPlugin : public flutter::Plugin {
 ...
 private:
  ...
  bool TrayManagerPlugin::_ApplyIcon();
...
void TrayManagerPlugin::SetIcon(
  ...
  if (nid.hIcon == nullptr) {
    result->Error("tray_icon_load_failed", "Unable to load tray icon");
    return;
  }

  if (!_ApplyIcon()) {
    result->Error("tray_icon_add_failed", "Unable to add tray icon");
    return;
  }
...
bool TrayManagerPlugin::_ApplyIcon() {
  BOOL applied = FALSE;
  if (tray_icon_setted) {
    applied = Shell_NotifyIcon(NIM_MODIFY, &nid);
  } else {
    ...
    applied = Shell_NotifyIcon(NIM_ADD, &nid);
  }
  ...
  tray_icon_setted = nid.hIcon != nullptr;
  return tray_icon_setted;
}
